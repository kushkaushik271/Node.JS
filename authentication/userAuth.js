...
const checkAccess = (allowedRoles = [], allowedPermissions = []) => {
  return async (req, res, next) => {
    try {
      ...
      const userPermissions = userPermissionDocs.map(
        (up) => up.permissionID.permissionName
      );
      ...
      if (!hasUserPermissionAccess && userPermissionDocs?.length > 0) {
        return res
          ...
          .json({
            message:
              "You don't have user permission to do that. Please contact the admin.",
          });
      }
      ...
      if (allowedPermissions.length > 0 && userRole) {
        ...
        const rolePermissions = rolePermissionDocs.map(
          (rp) => rp.permissionID.permissionName
        );
      ...
      if (!hasRolePermissionAccess && rolePermissionDocs?.length > 0) {
        return res
          ...
          .json({
            message:
              "You don't have role permission to do that. Please contact the admin.",
          });
      }
