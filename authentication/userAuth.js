  ...
  return async (req, res, next) => {
    try {
      ...
      if (allowedPermissions.length > 0 && userRole) {
        rolePermissionDocs = await RolePermission.find({ roleID: userRole.roleID._id }).populate("permissionID");
        console.log({rolePermissionDocs})
        const rolePermissions = rolePermissionDocs.map(rp => rp.permissionID.permissionName);
        console.log({rolePermissions})
        hasRolePermissionAccess = allowedPermissions.some(permission => rolePermissions.includes(permission));
      }

      if (!hasRolePermissionAccess && rolePermissionDocs?.length > 0) {
        return res.status(403).json({ message: "You don't have role permission to do that. Please contact the admin." });
      }
