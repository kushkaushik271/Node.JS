...
const createPermission = async (req, res, next) => {
  try {
    ...
    const response = await roleService.createPermission(
      permissionName,
      description
    );
    res.status(200).json(response);
...
const assignPermissionToRole = async (req, res, next) => {
  try {
    ...
    const response = await roleService.assignPermissionToRole(
      roleId,
      permissionId,
      resourceId
    );
...
const assignPermissionToUser = async (req, res) => {
  try {
    ...
    const response = await roleService.assignPermissionToTheUser(
      userID,
      permissionID
    );
    res.status(201).json(response);
